function [err_message, ModalCoeffs, HasoSlopes, Phase] = Imop_Compute_PhaseModalZonal(ComputePhaseSet, HasoData, ModalCoeffs, HasoSlopes, Phase)

% IMOP_COMPUTE_PHASEMODALZONAL Compute phase from slopes using both zonal and modal reconstruction.
...
%    The difference HasoSlopes between processed slopes of the input HasoData and reconstructed slopes is called residual slopes.
...
%                     
%    HasoData         HasoData object adress containing the input slopes.
%                     
%    ModalCoeffs      ModalCoef object adress. Preferences are used as input, coefficients values are updated by the processing.
%                     
%    HasoSlopes       HasoSlopes object adress containing the residual slopes.
%                     
...
if nargin ~= 5
	error('The "Imop_Compute_PhaseModalZonal" function requires 5 input argument(s).');
end
err_message = char(ones(256,1,'int8'));

[err_nb, err_message] = calllib('WaveKitRuntime', 'Imop_Compute_PhaseModalZonal', err_message, ComputePhaseSet, HasoData, ModalCoeffs, HasoSlopes, Phase);
